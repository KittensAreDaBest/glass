          ...
          <div className='flex flex-col gap-4 md:gap-0 md:flex-row md:items-stretch'>
            ...
            <button
              ...
              onClick={() => {
                ...
                let type = ""
                if (inputRef.current?.value.match(/^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])(\/([0-9]|[1-2][0-9]|3[0-2]))?$/gm)) {
                  type = "ipv4"
                }
                ...
                if (inputRef.current?.value.match(/^([a-z0-9]+(-[a-z0-9]+)*\.)+[a-z]{2,}$/)) {
                  type = "domain"
                }
                ...
                if (method) {
                  ...
                  if (!method.supported.includes(type)) {
                    if (type === "domain") {
                      setDnsLoading(true)
                      setDomain(inputRef.current?.value ? inputRef.current?.value : "")
                      setIpv4([])
                      setIpv6([])
                      axios.get(`https://cloudflare-dns.com/dns-query?name=${inputRef.current?.value}&type=A`, {
                        headers: {
                        ...
                        axios.get(`https://cloudflare-dns.com/dns-query?name=${inputRef.current?.value}&type=AAAA`, {
                          headers: {
                            accept: "application/dns-json"
                          }
                        }).then(res => {
